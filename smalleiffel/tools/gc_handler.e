   ...
   memory_dispose(code: STRING; o: STRING; run_class: RUN_CLASS) is
      ...
      do
         ...
         if rf3 /= Void then
            code.append(once "if((");
            code.append(o);
            code.append(once "->header.flag)==FSOH_UNMARKED){");
            no_check := ace.no_check;
            if no_check then
               code.append(
               once "%N{se_frame_descriptor gcd={%"Garbage Collector at work.\n%
            ...
            code.append(fz_14);
            if no_check then
               code.append(once "se_dst=ds.caller;/*unlink*/%N}");
            end;
            code.extend('}');
         end;
      end;
   ...
   allocation_in(var, buffer: STRING; rc: RUN_CLASS) is
      ...
      do
	    ...
	    id.append_in(buffer);
         ...
         elseif var = fz_eiffel_root_object then
	    buffer.append(fz_b7);
	    id.append_in(buffer);
            buffer.append(
               once "*)se_malloc(sizeof(double)+sizeof(*eiffel_root_object)));%N%
	 ...
	 -- Code to create a new Manifest STRING in the "s" local C variable.
      do
         if is_off or else not string_at_run_time then
            ...
            c_code.append(once "s=((T7*)se_malloc(sizeof(gc7)));%N%
   ...
   mark_for(c_code: STRING; entity: STRING; rc: RUN_CLASS) is
      ...
      do
         ...
         if r /= Void then
	       ...
	       -- exapanded class must not have more than one runnable.
	       -- *** Fri Aug 15 2000, DC
