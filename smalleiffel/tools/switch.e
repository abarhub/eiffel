	 ...
	 tag: STRING;
      do
	    ...
	    cpp.put_string(fn.to_string);
            cpp.put_string(once " (late bind.)%",0,0,%"%",1};%N%
		           %se_dump_stack ds;%N%
			   %ds.fd=&fd;%Nds.p=0;%N%
                           %ds.caller=caller;%N");
         end;
         ...
         if not boost then
            cpp.put_string(once "se_dst=caller;%N");
         end;
         if result_type /= Void then
...
feature {C_PRETTY_PRINTER}
   ...
   put_ith_argument(up_rf: RUN_FEATURE; fal: FORMAL_ARG_LIST; i: INTEGER) is
	 ...
	 if conversion then
	    assignment_handler.c_function_call(at,ft);
	 end;
	 cpp.put_character('a');
	 cpp.put_integer(i);
	 if conversion then
...
feature {SWITCH_COLLECTION}
   ...
   afd(up_rf: RUN_FEATURE) is
      local
         r: ARRAY[RUN_CLASS];
         i: INTEGER;
         x_result_type, r_result_type: E_TYPE;
         x_current_type, r_current_type: E_TYPE;
         dyn_rf: RUN_FEATURE;
      ...
      do
         ...
         loop
            ...
            assignment_handler.passing(x_current_type,r_current_type);
            if x_result_type /= Void then
               r_result_type := dyn_rf.result_type;
               assignment_handler.passing(r_result_type,x_result_type);
            end;
...
feature {NONE}
   ...
   jvm_switch(up_rf: RUN_FEATURE; rt: E_TYPE) is
