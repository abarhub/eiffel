...
feature
   ...
   run_time_trace: BOOLEAN is
	 ...
	 bc := base_class;
	 if bc /= Void then
	    Result := bc.run_time_trace;
	 end;
      end;

   show is
      require
	 not small_eiffel.no_file_for(base_class_name.to_string)
      local
	 ...
	 if file_path = Void and then id = 0 then
   ...
   set_in(bc: like base_class) is
	 ...
	 -- code of class TUPLE which is not truly written.
      require
	 small_eiffel.no_file_for(bc.name.to_string)
      do
...
feature {EIFFEL_PARSER}
   ...
   eiffel_file(li, co, class_id: INTEGER; bc: like base_class) is
