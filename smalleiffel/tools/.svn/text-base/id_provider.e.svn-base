...
feature {COMPILE_TO_C}
   ...
   disk_save is
      local
         i, id: INTEGER; tfw: TEXT_FILE_WRITE; str: STRING;
      do
         !!tfw.make;
         ...
         loop
            ...
            tfw.put_character('%"');
            tfw.put_character(' ');
            small_eiffel.id_extra_information(tfw,str);
            tfw.put_character('%N');
            i := i + 1;
         end;
         tfw.disconnect;
...
feature {PARSER,BASE_CLASS,RUN_CLASS}

   item(str: STRING): INTEGER is
      require
         str = string_aliaser.item(str)
      do
	 Result := id_memory.reference_at(str);
         if Result = 0 then
            max_id := max_id + 1;
            Result := max_id;
...
feature {NONE}
   ...
   make is
      do
         ...
         id_memory.put(11,as_gui_color);
         id_memory.put(12,as_gui_event);
         id_memory.put(13,as_gui_font);
         id_memory.put(14,as_gui_gc);
         id_memory.put(15,as_gui_pixmap);
         max_id := 15;
         disk_restore;
      end;
      ...
      do
	    ...
	    if state = 6 then
	       ...
	       echo.put_string(" correct items).%N");
            ...
            echo.put_string(once "Previous IDs reloaded (");
