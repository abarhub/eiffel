   ...
   c_define_for(ma_type: E_TYPE) is
      ...
      do
         ...
         if (elt_type.is_integer or else
          ...
          then
               body.append(once "*(s++)=va_arg(pa,int);%N");
         elseif elt_type.is_real or else elt_type.is_double then
               body.append(once "*(s++)=va_arg(pa,double);%N");
         elseif elt_type.is_user_expanded then
            body.append(
             once "memcpy(s++,va_arg(pa,char*),sizeof(*s));%N");
         else
            body.append(once "*(s++)=((void*)(va_arg(pa,char*)));%N");
         end;
         body.append(once "}%N%
                     %va_end(pa);%N}%N%
                     %else{%N%
