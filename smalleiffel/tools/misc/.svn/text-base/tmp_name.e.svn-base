...
expanded class TMP_NAME
   ...
   -- storage of some unkown name during syntax analysis.
   --
...
feature {PARSER}
   ...
   to_infix_name_use: FEATURE_NAME is
      do
         create Result.infix_name(aliased_string,start_position);
      end;

   to_infix_name(sp: POSITION): FEATURE_NAME is
      do
         create Result.infix_name(aliased_string,sp);
      end;
   ...
   to_prefix_name: FEATURE_NAME is
      do
         create Result.prefix_name(aliased_string,start_position);
      end;
...
feature {NONE}
   ...
   new_keyword_warning is
	 ...
	 eh.extend('%"');
	 eh.append(buffer)
	 eh.append("%" is now a keyword in the new Eiffel definition. %
