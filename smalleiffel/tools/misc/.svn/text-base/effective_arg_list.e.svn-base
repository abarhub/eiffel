...
feature {SMALL_EIFFEL}
   ...
   match_with(rf: RUN_FEATURE) is
      ...
      require
         rf.arguments /= Void
      local
         fal: FORMAL_ARG_LIST; i: INTEGER; e: EXPRESSION;
         at, ft: E_TYPE;
      do
         from
	    fal := rf.arguments;
	    ...
	    if fal.count /= count then
	       ...
	       eh.append(". (The actual argument list has ");
	       eh.append_integer(count);
	       eh.append(" item");
	       if count > 1 then eh.extend('s') end;
	       eh.append(".)");
...
feature {E_AGENT,EFFECTIVE_ARG_LIST}
   ...
   open_operands_notify(e_agent: E_AGENT; rf: RUN_FEATURE): like Current is
	 -- Before the traditional `to_runnable', we have to notify open operands.
      local
...
feature {NONE}
   ...
   is_like_argument(e: EXPRESSION; at, ft: E_TYPE): BOOLEAN is
      local
         tla: TYPE_LIKE_ARGUMENT; ot: E_TYPE;
      do
         tla ?= ft;
         if tla /= Void then
            Result := true;
