...
feature {RUN_FEATURE}
   ...
   define_body_of(rf: RUN_FEATURE) is
	 ...
	 eal: EFFECTIVE_ARG_LIST; running: ARRAY[RUN_CLASS];
      do
         rfct := rf.current_type;
         rfrt := rf.result_type;
         running := rfct.run_class.running;
         if running = Void then
            eh.add_type(rfct,once " not created (type is not alive).");
            eh.append(once " Empty Cecil/Wrapper function ");
            ...
            if ace.no_check then
               cpp.put_string(
               once "se_dump_stack ds={NULL,NULL,0,NULL,NULL};%N%
               %ds.caller=se_dst;%N%
               %se_dst=&ds;%N");
            end;
            if rfrt /= Void then
            ...
            !!face_target.make(rf.current_type);
            ...
            if rfct.is_expanded then
               ...
               cpp.push_cpc(rf,running,face_target,eal);
            end;
            if rfrt /= Void then
               cpp.put_string(fz_00);
            end;
            if ace.no_check then
               cpp.put_string(once "se_dst=ds.caller;%N");
            end;
            if rfrt /= Void then
