...
feature
   ...
   read_character is
      do
         if push_back_flag then
            ...
            memory := basic_io_getc(basic_io_stdin);
         end;
      end;
   ...
   read_line_in(str: STRING) is
      ...
      do
         read_character;
         if last_character /= '%N' and then memory /= basic_io_eof then
            from
               str.extend(memory.to_character);
               mem := basic_io_getc(basic_io_stdin);
            until
               mem = basic_io_eof or else mem = ('%N').code
            loop
