...
--
-- The SmallEiffel standard library provides four implementations of
...
feature -- Looking and comparison:
   ...
   frozen nb_occurrences(element: like item): INTEGER is
      obsolete "Use now `occurrences' to be uniform with STRING ELKS20001"
      do
	 Result := occurrences(element);
      end;
   ...
   frozen fast_nb_occurrences(element: like item): INTEGER is
      obsolete "Use now `fast_occurrences' to be uniform with STRING ELKS20001"
      do
	 Result := fast_occurrences(element);
      end;
