...
feature -- Conversions and printing:
   ...
   frozen to_string: STRING is
	 ...
	 -- Note: see also `append_in' to save memory.
      do
	 Result :=  "......... (This is a private buffer) ............";
	 Result.clear;
	 append_in(Result);
	 Result := Result.twin;
      end;
   ...
   append_in(buffer: STRING) is
	 -- Append the equivalent of `to_string' at the end of `buffer'. Thus you
	 -- can save memory because no other STRING is allocated for the job.
      require
	 buffer /= Void
      deferred
      end;
   ...
   frozen to_decimal(digits: INTEGER; all_digits: BOOLEAN): STRING is
         -- Convert `Current' into its decimal view. A maximum of decimal `digits'
         -- places will be used for the decimal part. If the `all_digits' flag is
         -- true insignificant digits will be included as well. (See also
         -- `decimal_in' to save memory.)
      require
   ...
   append_decimal_in(buffer: STRING; digits: INTEGER; all_digits: BOOLEAN) is
	 -- Append the equivalent of `to_decimal' at the end of `buffer'. Thus you
	 -- can save memory because no other STRING is allocated for the job.
      require
	 non_negative_digits: digits >= 0
      deferred
...
feature {NONE}
   ...
   min_double: NUMBER is
	 ...
	 nt: NUMBER_TOOLS;
      once
	 Result := nt.from_string(Minimum_double.to_string);
      end;
