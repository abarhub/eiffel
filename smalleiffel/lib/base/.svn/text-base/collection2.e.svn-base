...
deferred class COLLECTION2[E]
   ...
   -- of type E.
   --
   -- The SmallEiffel standard library (SmallEiffel/lib/base) provides two
...
feature -- Miscellaneous features:
   ...
   frozen nb_occurrences(elt: E): INTEGER is
      obsolete "Use `occurrences' instead"
      do
	 Result := occurrences(elt);
      end;
   ...
   frozen fast_nb_occurrences(elt: E): INTEGER is
      obsolete "Use `fast_occurrences' instead"
      do
	 Result := fast_occurrences(elt);
      end;
