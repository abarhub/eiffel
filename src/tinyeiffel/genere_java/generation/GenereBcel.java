...
public class GenereBcel implements GenereJvm {
	...
	public boolean verification(String rep) {
		...
		if(liste!=null)
		{
			...
			nb_erreur=0;
			nb_warning=0;
			liste_erreurs=new ArrayList();
			System.out.println("Vérification des classes...");
			for(i=0;i<liste.size();i++)
			{
				j=(JavaClass)liste.get(i);
				Repository.addClass(j);
				System.out.println("Classe "+j.getClassName()+" ajoutée");
			}
			ajout_classe_supplementaire(rep);
			for(i=0;i<liste.size();i++)
			{
				j=(JavaClass)liste.get(i);
				...
				if (vr == VerificationResult.VR_OK){
					...
					if (vr == VerificationResult.VR_OK){
						...
						for (int k=0; k<jc.getMethods().length; k++){
							nom_methode=jc.getMethods()[k].getName()+jc.getMethods()[k].getSignature();
							vr = v.doPass3a(k);
							if(vr.getStatus()!=VerificationResult.VERIFIED_OK)
							{
								nb_erreur++;
								liste_erreurs.add(nom_classe+"."+nom_methode+" (passe 3a) : "+vr);
							}
							...
							{
								nb_erreur++;
								liste_erreurs.add(nom_classe+"."+nom_methode+" (passe 3b) : "+vr);
							}
					...
					{
						nb_erreur++;
						liste_erreurs.add(nom_classe+" (passe 2) : "+vr);
					}
				}
				else
				{
					nb_erreur++;
					liste_erreurs.add(nom_classe+" (passe 1) : "+vr);
				}
			...
			{
				System.out.println("**********************************");
				System.out.println("Liste des erreurs ("+nb_erreur+"):");
				for(i=0;i<liste_erreurs.size();i++)
				{
					System.out.println("#####");
					System.out.println(liste_erreurs.get(i));
				}
				System.out.println("("+nb_erreur+" erreurs) **********************************");
			}
			System.out.flush();
			return nb_erreur==0;
		}
	...
	private void ajout_classe_supplementaire(String rep) {
		ClassParser parser;
		try {
			parser=new ClassParser(rep+"Utils.class");
	...
	public int instr_cast(CIType src, CIType dest) {
		return ajoute_instr(factory.createCast(conv_type(src),conv_type2(dest)));
	}

	private Type conv_type2(CIType type) {
		if(type==null)
			return Type.VOID;
		else if(env.est_special(type))
			return env.conv_type(type);
		else
			return new ObjectType(((CITypeSimple)type).nom);
		//conv_type(t);
		//return null;
	}
